...
public class PortalTable extends MultiBlockMachine {
    ...
    public static final RecipeType PORTAL_TABLE_RECIPETYPE = new RecipeType(
            ...
            "",
            BukkitComponentSerializer.legacy().serialize(ElementManipulation.MM.deserialize("<rainbow:3>这是以最亚原子的方式</rainbow>")),
            BukkitComponentSerializer.legacy().serialize(ElementManipulation.MM.deserialize("<rainbow:2>操纵原子的正确地方</rainbow>")),
            BukkitComponentSerializer.legacy().serialize(ElementManipulation.MM.deserialize("<rainbow:1>并且为跨维度入口建造任意门</rainbow>"))
    );
